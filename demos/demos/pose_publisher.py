"""pose_publisher.py
   ...
   Publish a pose: x/y/z position and roll/pitch/yaw angles, as set by
   six GUI sliders.

   This generates a pose message, as well as the transform from
   'world' to 'gui', which rviz can show (under TF).

   Node:        /pose
   Publish:     /pose                   geometry_msgs.msg.PoseStamped
   Broadcast:   'world' -> 'gui'        geometry_msgs.msg.TransformStamped
"""
...
class GUINode(Node):
    ...
    def __init__(self, name, rate):
        ...
        self.publisher = self.create_publisher(PoseStamped, '/pose', quality)
        ...
        self.transmsg.header.frame_id = 'world'
        self.transmsg.child_frame_id  = 'gui'
    ...
    def run(self):
        ...
        app.setApplicationDisplayName("Pose Publisher")
    ...
    def publish(self):
        ...
        # Publish the pose and broadcast the transform.
        self.posemsg.header.stamp  = stamp
        self.transmsg.header.stamp = stamp
        self.publisher.publish(self.posemsg)
        self.broadcaster.sendTransform(self.transmsg)


    # Callback from the GUI to set the pose.
    def CB(self, v):
        # Pre-populate the position and orientations.
        point      = Point(x=v[0], y=v[1], z=v[2])
        vector3    = Vector3(x=v[0], y=v[1], z=v[2])
        quaternion = Quaternion_from_R(R_from_RPY(v[3], v[4], v[5]))
        
        # Update the messages.
        self.posemsg.pose.position          = point
        self.posemsg.pose.orientation       = quaternion
        self.transmsg.transform.translation = vector3
        self.transmsg.transform.rotation    = quaternion
