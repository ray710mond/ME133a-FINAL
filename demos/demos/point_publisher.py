...
class GUINode(Node):
    ...
    def __init__(self, name, rate):
        ...
        self.markermsg.header.frame_id  = "world"
        self.markermsg.header.stamp     = self.get_clock().now().to_msg()

        self.markermsg.action           = Marker.ADD
        self.markermsg.ns               = "point"
        self.markermsg.id               = 1
        self.markermsg.type             = Marker.SPHERE
        self.markermsg.pose.orientation = Quaternion()
        self.markermsg.pose.position    = self.p
        self.markermsg.scale            = Vector3(x=0.2, y=0.2, z=0.2)
        self.markermsg.color            = ColorRGBA(r=1.0, g=0.0, b=0.0, a=0.8)
        # a = 0.8 is slightly transparent!

        # Create the marker array message.
        self.markerarraymsg = MarkerArray(markers = [self.markermsg])
    ...
    def publish(self):
        ...
        self.markermsg.header.stamp = stamp
        self.pointmsg.header.stamp  = stamp
        self.pub_mark.publish(self.markerarraymsg)
        self.pub_point.publish(self.pointmsg)


    # Callback from the GUI to set the point.
    def CB(self, v):
        # Update the point.
        self.p.x = v[0]
        self.p.y = v[1]
